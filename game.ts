...
function updateStatus() {
  ...
  const hpBar = document.getElementById("player-health-bar")! as HTMLElement;
  const staminaBar = document.getElementById("player-stamina-bar")! as HTMLElement;
...
function nextEnemy() {
  ...
  log(
    `⚔️ Um novo inimigo apareceu! Nível ${enemy.level}, Vida ${enemy.hp}. Você subiu para o nível ${player.level} e recuperou ${recoverHp} de vida.`
  );
  updateStatus();
}

(window as any).playerAttack = function playerAttack() {
...
(window as any).playerMagic = function playerMagic() {
  if (player.spells.length === 0) {
    player.spells = [...magicList].sort(() => 0.5 - Math.random()).slice(0, 4);
    log(
      "🔮 Suas magias disponíveis: " + player.spells.map((m) => m.name).join(", ")
    );
  }
...
(window as any).playerFlee = function playerFlee() {
  if (Math.random() < 0.5) {
...
function enemyTurn() {
  if (enemy.hp <= 0) return nextEnemy();
...
window.addEventListener("keydown", (event) => {
  if (event.key.toLowerCase() === "r") {
    const buttons = document.querySelectorAll(".btn");
    if (Array.from(buttons).every((b) => (b as HTMLButtonElement).disabled)) {
